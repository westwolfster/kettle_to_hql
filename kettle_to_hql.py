...
def process_hive_sql(sql: str, table_map: Dict[str, str]) -> Tuple[str, Set[str]]:
    ...
    规则 2.5：仅改写「映射旧表」上的 set_day / set_month 谓词为 1=1。
    未出现在 Excel 对照表中的表，其分区条件原样保留。
    INSERT/ALTER 的 PARTITION (set_day=...) 不改写。
    """
    ...
    # 记录：被映射的旧表 以及 这些表在 SQL 中使用的别名
    mapped_aliases: Set[str] = set()  # 小写别名，如 {'a', 'a1', 'c1'}
    mapped_bare = False  # 是否存在无别名的映射表（FROM mapped_table 无 alias）

    def repl_table(m: re.Match) -> str:
        nonlocal mapped_bare
        prefix, raw = m.group(1), m.group(2)
        is_mapped = is_mapped_old_table(raw, table_map)
        ...
        # 尝试捕获紧跟表名后的别名：  table alias  或  table AS alias
        # 在整段 SQL 上用另一遍扫描更可靠，这里先只做表名替换
        return prefix + new

    sql = TABLE_REF_RE.sub(repl_table, sql)

    # 第二遍：根据「原 SQL 表名 + 别名」识别映射表别名
    # 匹配 FROM/JOIN 后的 表名 [AS] 别名
    alias_re = re.compile(
        r"""(?ix)
        (?:from|join)\s+
        ([`"]?[\w\.\$\{\}]+[`"]?)
        (?:\s+as)?\s+
        ([a-z][a-z0-9_]*)
        (?=\s|$|,|on|where|left|right|inner|full|join|group|order|limit|union|\)|;)
        """,
    )
    # 注意：此时表名可能已被 rewrite，需同时用原名和映射后的名判断
    # 更稳妥：在 rewrite 之前扫描别名。这里对 rewrite 后的 SQL，
    # 用 is_mapped 对「rewrite 前」无法判断；改为：凡是最终落到
    # ap_tenant_user7_dev.<excel新表名> 的，视为映射表。
    excel_new_names = set(table_map.values())  # 新表裸名

    for m in alias_re.finditer(sql):
        tbl_raw = m.group(1).strip('`"').lower()
        alias = m.group(2).lower()
        bare = tbl_raw.split(".")[-1]
        # 映射表特征：在 excel 新表名中，或仍能通过 is_mapped_old_table 命中
        if bare in excel_new_names or is_mapped_old_table(tbl_raw, table_map):
            mapped_aliases.add(alias)

    # 无别名的 FROM/JOIN 映射表
    bare_tbl_re = re.compile(
        r"""(?ix)
        (?:from|join)\s+
        ([`"]?[\w\.\$\{\}]+[`"]?)
        (?=\s*(?:on|where|left|right|inner|full|join|group|order|limit|union|,|\)|;|$))
    ...
    for m in bare_tbl_re.finditer(sql):
        tbl_raw = m.group(1).strip('`"').lower()
        bare = tbl_raw.split(".")[-1]
        if bare in excel_new_names or is_mapped_old_table(tbl_raw, table_map):
            mapped_bare = True

    def should_kill(alias: str) -> bool:
        alias = (alias or "").rstrip(".").lower()
        if alias:
            return alias in mapped_aliases
        # 无别名：仅当存在无别名映射表时才改
        return mapped_bare

    # 改写 WHERE/AND/ON 中的 set_day/set_month
    # 不改写 PARTITION (set_day = ...) —— 用负向后瞻排除 PARTITION 关键字
    part_pred = re.compile(
        r"""(?ix)
        (?<!partition\s)(?<!partition)  # 不匹配 PARTITION 子句（尽力）
        (?P<lead>
            (?:(?:and|where|on)\s*\(?\s*)
            |
            (?:\(\s*)
    ...
    def kill_and(m: re.Match) -> str:
        alias = m.group("alias") or ""
        if should_kill(alias):
            return m.group("pre") + "1 = 1"
        return m.group(0)

    sql = and_pred.sub(kill_and, sql)
