...
# 关键字后的表名，以及逗号风格 JOIN 中带 schema 的表名（from a, schema.tbl b）
TABLE_REF_RE = re.compile(
    ...
    (?:
        ...
        ([`"]?[\w\.\$\{\}]+[`"]?)
      |
        # 2) 逗号风格多表：仅匹配 schema.table，避免误伤 SELECT 列清单
        (,\s*)
        ([`"]?[a-z_][\w]*_[\w]*(?:\.[\w\.\$\{\}]+)+[`"]?)
    )
    """,
...
def rewrite_table_name(raw: str, table_map: Dict[str, str]) -> str:
    """统一输出小写；模式名一律以 _dev 结尾；临时表以 temp_ 开头。

    1. 无模式名 → ap_tenant_user7_dev.{表名}
    2. 有模式名且命中对照表 → ap_tenant_user7_dev.{table_name_new}
    3. 有模式名未命中对照表 → {schema}_dev.{表名}
    4. 临时表（tmp_/temp_）→ ap_tenant_user7_dev.temp_{去前缀后的名}
    """
    name = raw.strip("`\"")
    lower = name.lower()
...
def process_hive_sql(sql: str, table_map: Dict[str, str]) -> Tuple[str, Set[str]]:
    ...
    规则：
    - 表名按 rewrite_table_name 统一改写（一律小写，模式名以 _dev 结尾）
    - 仅将「映射表别名」上的 set_day/set_month 谓词改为 1=1
    - 未映射表别名的分区条件原样保留；PARTITION 写入规格不改动
    """
    ...
    # 改写前：收集映射表的别名（不用裸逗号，避免 SELECT 列清单误匹配）
    mapped_aliases: Set[str] = set()
    _alias_skip = {
        "where", "join", "left", "right", "inner", "outer", "full",
        "cross", "on", "and", "or", "group", "order", "union",
        "having", "limit", "select", "from", "as", "set",
    }
    for m in re.finditer(
        ...
        raw_tbl, alias = m.group(1), m.group(2)
        if alias.lower() in _alias_skip:
            continue
        if is_mapped_old_table(raw_tbl, table_map):
            mapped_aliases.add(alias.lower())
    # 逗号风格 JOIN：仅 schema 含下划线的表
    for m in re.finditer(
        r"""(?ix)
        ,\s*
        ([`"]?[a-z_][\w]*_[\w]*(?:\.[\w\.\$\{\}]+)+[`"]?)
    ...
    def repl_table(m: re.Match) -> str:
        # group1+2: 关键字后的表；group3+4: 逗号后的 schema.table
        if m.group(1) is not None:
            prefix, raw = m.group(1), m.group(2)
        else:
            prefix, raw = m.group(3), m.group(4)
    ...
    # 按别名精确替换 set_day/set_month（词边界避免误匹配 PARTITION）
    part_pred = re.compile(
        r"""(?ix)
        (?P<lead>(?<![a-z_])(?:and|where|on)\s+\(?\s*)
        (?P<alias>[a-z_][\w]*\.)?
    ...
    def kill_or_keep(m: re.Match) -> str:
        alias = (m.group("alias") or "").rstrip(".").lower()
        if alias:
            # 有别名：仅当该别名对应映射表时才替换
            if alias in mapped_aliases:
                return m.group("lead") + "1 = 1"
            return m.group(0)
        # 无别名：仅当语句中存在映射表时替换（兼容单表语句）
        if mapped_originals:
            return m.group("lead") + "1 = 1"
        return m.group(0)
